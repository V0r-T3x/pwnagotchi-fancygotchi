...
class fancygotchi(object):
    ...
    def fancy_change(self, partial=False, fancy_dict=[]):
        ...
        if th_opt['bg_anim_image'] != '':
            ...
            gif = Image.open('%simg/%s' % (pwnagotchi._fancy_theme, th_opt['bg_anim_image']))
            self._frames = []
            for frame in ImageSequence.Iterator(gif):
                self._frames.append(frame.convert("RGBA"))
        if th_opt['bg_image'] != '':
            self._bg = Image.open('%simg/%s' % (pwnagotchi._fancy_theme, th_opt['bg_image']))
            self._bg = self._bg.convert('RGBA')
        if th_opt['fg_image'] != '':
            self._fg = Image.open('%simg/%s' % (pwnagotchi._fancy_theme, th_opt['fg_image']))
                    ...
                    if isiconic:
                        ...
                        for face_name, face_value in th_faces.items():
                            ...
                            if os.path.isfile(icon_path):
                                face_image = self.adjust_image(icon_path, zoom, mask)
                            else:
                                #logging.warning('[FANCYGOTCHI] missing the %s image' % (face_name))
                                face_image = self.adjust_image(icon_broken, zoom, mask)
    ...
    def adjust_image(self, mage_path, zoom, mask=False):
        try:
            # Open the image
